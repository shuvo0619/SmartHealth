...
namespace SmartHealth.Hubs
{
   [HubName("signalRChatHub")]
    ...
    {
        #region---Data Members---
        static List<UserAndRole> ConnectedUsers = new List<UserAndRole>();
        static List<Message> CurrentMessage = new List<Message>();
        
        #endregion

        SqlConnection sqlcon = new SqlConnection(ConfigurationManager.ConnectionStrings["SmartHealth_Entities"].ConnectionString);
        ...
        public void Send_PrivateMessage(String msgFrom, String msg, String UserId)
        {
            var id = Context.ConnectionId;
            //msgFrom = HttpContext.Current.User.Identity.Name;
            Clients.Caller.receiveMessage(msgFrom, msg, UserId);
            Clients.Client(UserId).receiveMessage(msgFrom, msg, id);
        ...
        public string updaterec(string username,string userid, string connectionid)
        {
            try
            {
                SqlCommand save = new SqlCommand("insert into [smart.Message] values('" + username + "','" + userid + "','" + connectionid + "')", sqlcon);
                sqlcon.Open();
        ...
        public int GetCount()
        {
            ...
            try
            {
                SqlCommand getCount = new SqlCommand("select COUNT([UserName]) as TotalCount from [smart.Message]", sqlcon);
                sqlcon.Open();
        ...
        public bool DeleteRecord(string connectionid)
        {
            ...
            try
            {
                SqlCommand deleterec = new SqlCommand("delete from [smart.Message] where ([ConnectionID]='" + connectionid + "')", sqlcon);
                sqlcon.Open();
        ...
        public string GetUsers(string username)
        {
            ...
            {
                int count = GetCount();
                SqlCommand listrec = new SqlCommand("select [UserName],[ConnectionID] from [smart.Message] where ([UserName]<>'" + username + "')", sqlcon);
                sqlcon.Open();
